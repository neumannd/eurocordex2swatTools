...
#' @param data data.frame: precipitation [mm]; contains a column with time stamp and >=1 columns with data; time column needs to be of type POSIXct
...
#' 
write_input_swat2012_pcp <- function(file, data, stations, station_names, col_time_name = 'time_posix', 
  ...
  header_str[1] <- paste0('Station  ', paste0('PCP_', station_names, collapse = ','), ',')
  
  # write station data header rows 2 to 4 ----
  header_str[2] <- paste0('Lati   ', paste0(formatC(stations[[col_lat]], format = 'f', width = 5, digits = 1), collapse=''))
  header_str[3] <- paste0('Long   ', paste0(formatC(stations[[col_lon]], format = 'f', width = 5, digits = 1), collapse=''))
  header_str[4] <- paste0('Elev   ', paste0(formatC(stations[[col_elev]], format = 'd', width = 5), collapse=''))
  ...
  data_str = paste0(format(data[[col_time_name]],"%Y%j"), 
                    apply(formatC(as.matrix(data[station_names]), format='f', width=5, digits=1, flag='0'), 
                          1, paste0, collapse = ''))
